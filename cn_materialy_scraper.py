...
E-mail UA (chiński) przez yagmail/Gmail w tym module (send_email_cn_materialy).
...
_paths = campaign_output_paths(_campaign, "cn_materialy")
...
def apply_cn_run_config_extras(module, data: dict) -> None:
...
import cn_materialy_supplier_filter as _retail_store_builder_filter
...
CAMPAIGN_TIMEZONE = os.environ.get("SCRAPER_TIMEZONE", "Asia/Shanghai")
...
from cn_materialy_inquiry_email_zh import FIXED_MATERIAL_INQUIRY_ZH, build_fixed_material_inquiry_zh
...
def get_email_attachments_cn_materialy(logger: logging.Logger | None = None) -> list[str]:
...
def setup_logging() -> logging.Logger:
    ...
    return setup_script_logging("cn_materialy_scraper", LOG_FILE)
...
def is_rejected_company_name_for_export(
    ...
    if any(m in low for m in _COMPANY_NAME_HARD_REJECT_MARKERS):
        return True
    from cn_contact_fields import is_pl_junk_company_name, is_pl_seo_title
...
def build_claude_row_cleanup_prompt(
    *,
    ...
    from cn_claude_prompts import build_row_cleanup_prompt
...
def apply_row_enrichment_to_row(row: dict, llm_result: dict) -> None:
    ...
    from cn_contact_fields import sanitize_export_address
...
def finalize_row_for_excel_tables(row: dict) -> dict:
    """Po Claude + regex: spójne pola pod arkusze Kontakte i Prowincje."""
...
def sanitize_sender_name(sender_name: str) -> str:
    from cn_materialy_inquiry_email_zh import inquiry_company_name, inquiry_sender_name
...
def campaign_today() -> str:
    """Dzień kampanii wg SCRAPER_TIMEZONE (domyślnie Asia/Shanghai)."""
    try:
...
def verify_company_on_website(
    ...
    *,
    ...
    if ENABLE_CLAUDE_PAGE_VERIFY:
        ...
        from cn_claude_page_verify import claude_verify_company_page
...
def should_prefer_domain_company_name(raw_name: str, website: str) -> bool:
    ...
    from cn_contact_fields import is_pl_seo_title
    ...
    *,
    ...
    for bucket in buckets:
        for item in data.get(bucket, []) or []:
            ...
            from cn_contact_fields import serper_discovery_address
...
def generate_email_content(
    ...
    *,
    ...
    if ENABLE_CLAUDE_INQUIRY_EMAIL:
        ...
        generated = claude_generate_inquiry_email_zh(
            display_name,
            logger,
            cache,
    ...
    *,
    ...
    for mail in jobs_to_send:
        ...
        if dry_run_email:
            ...
            ok, info = send_email_cn_materialy(target, subject, body, logger)
...
def run_scraper(
    ...
    **_deprecated_kwargs,
    ...
    rotate_mode = bool(rotate_province and discovery_mode != "emails_only")
    from cn_province_rotation import rotation_is_active

    if rotate_province and discovery_mode != "emails_only":
    ...
    if rotate_province and discovery_mode != "emails_only" and not rebuild_from_cache:
        ...
        if rotation_is_active():
            ...
            print(
                f"[ROTACJA] Discovery dla prowincjia: {rotation_land} "
                f"(1 prowincjio / cykl, min. nowych firm={MIN_CONTACTS_TARGET})"
            )
            ...
            print(
                ...
                f"{len(CAMPAIGN_ACTIVE_BUNDESLAENDER)} prowincji."
            )
    ...
    if (
        ...
        from cn_province_rotation import commit_rotation_after_run
...
if __name__ == "__main__":
    ...
    if "--test" in sys.argv:
        ...
        if "--province" in sys.argv or "--bundesland" in sys.argv:
            i = sys.argv.index("--province") if "--province" in sys.argv else sys.argv.index("--bundesland")
            if i + 1 < len(sys.argv):
                bl = sys.argv[i + 1].split(",")
                configure_campaign_bundeslaender(sys.modules[__name__], bl)
                print(f"[TRYB] Aktywne prowincjia: {', '.join(CAMPAIGN_ACTIVE_BUNDESLAENDER)}")
        if "--rotation-status" in sys.argv:
            from cn_province_rotation import format_rotation_status

            print(format_rotation_status(OUTPUT_DIR))
            raise SystemExit(0)
        rotate_bl = "--rotate-province" in sys.argv
        if rotate_bl:
            extra_kw["rotate_province"] = True
        ...
        if rc_path:
            ...
            try:
                with open(Path(rc_path), encoding="utf-8") as _rcf:
                    apply_cn_run_config_extras(mod, json.load(_rcf))
            except Exception:
                pass
            launch_kw = run_scraper_launch_kwargs(mod)
