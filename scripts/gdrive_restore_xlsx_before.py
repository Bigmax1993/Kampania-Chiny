...
def _canonical_name(campaign: str) -> str:
    if campaign in ("cn", "pl"):
        return "cn_materialy_kontakte.xlsx"
    if campaign == "ua":
...
def _artifact_xlsx_before(cutoff: datetime, campaign: str, dest: Path) -> Path | None:
    ...
    if not repo or not token:
        ...
        return None
    prefix = "cn-materialy-wyniki-" if campaign in ("cn", "pl") else "ua-materialy-wyniki-"
    proc = subprocess.run(
        [
            "gh",
            "api",
...
def main() -> int:
    ...
    parser.add_argument("--campaign", choices=("cn", "ua", "gu", "pl"), default="cn")
    parser.add_argument("--before", default="2026-08-19", help="YYYY-MM-DD (Asia/Shanghai, wylacznie)")
