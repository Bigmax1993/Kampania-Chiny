...
import cn_materialy_scraper as scraper  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser(description="Wyślij jeden testowy mail kampanii CN")
    ...
    parser.add_argument(
        "--run-config",
        default="run_config/cn_materialy.json",
        help="Plik run_config kampanii CN",
    )
    args = parser.parse_args()

    data = load_run_config_file(args.run_config, ROOT)
    apply_run_config_file(scraper, args.run_config, ROOT)
    scraper.apply_cn_run_config_extras(scraper, data)
    ...
    ok, info = scraper.send_email_cn_materialy(args.to, subject, body, logger)
    if not ok:
