...
class GdriveSkipUploadTest(unittest.TestCase):
    def test_skip_json_and_log(self):
        self.assertTrue(_skip_gdrive_upload(Path("cn_materialy_cache.json")))
        self.assertTrue(_skip_gdrive_upload(Path("cn_materialy_scraper.log")))
        self.assertTrue(_skip_gdrive_upload(Path("cn_materialy_wojewodztwo_rotation.JSON")))

    def test_upload_xlsx_and_eml(self):
        self.assertFalse(_skip_gdrive_upload(Path("cn_materialy_kontakte.xlsx")))
